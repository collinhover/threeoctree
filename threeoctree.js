...
( function( THREE ) {
	...
	THREE.Octree.prototype = {
		...
		search: function( position, radius, organizeByObject, direction ) {
			...
			if ( organizeByObject === true ) {
				...
				for ( i = 0, l = objects.length; i < l; i ++ ) {
					...
					// if object data has a face or a vertex, add it to the result data

					if ( objectData.face ) {

						resultData.faces.push( objectData.face );

					} else if ( objectData.vertex ) {

						resultData.vertices.push( objectData.vertex );
		...
		raycast: function( raycaster, intersects ) {
			...
			var octreeResults = this.search( raycaster.ray.origin, raycaster.ray.far, false, raycaster.ray.direction );

			var objects = raycaster.intersectObjects( octreeResults, false );

			Array.prototype.push.apply( intersects, objects );
	...
	THREE.OctreeObjectData = function( object, part ) {
		...
		switch ( true ){
			...
			case part instanceof THREE.Face3:
				this.face = part;
				break;

			case part instanceof THREE.Vector3:
				this.vertex = part;
				break;
	...
	THREE.OctreeObjectData.prototype = {
		...
		update: function() {
			...
			if ( this.face ) {
				...
				this.computeFaceCentroid();
				this.computeFaceBoundingRadius();
				this.position.copy( this.centroid );
				this.object.localToWorld( this.position );

			} else if ( this.vertex ) {

				this.computeVertexBoundingRadius();
				this.position.copy( this.vertex );
				this.object.localToWorld( this.position );

			} else {

				this.computeBoundingRadius();
				this.position.copy( this.object.geometry.boundingSphere.center );
				this.object.localToWorld( this.position );
		...
		computeFaceCentroid: function() {

			var geometry = this.object.geometry || this.object,
				vertices = geometry.vertices,
				face = this.face;

			this.centroid = new THREE.Vector3();
			this.centroid.add( vertices[ face.a ] );
			this.centroid.add( vertices[ face.b ] );
			this.centroid.add( vertices[ face.c ] );
			this.centroid.divideScalar( 3 );

		},

		computeFaceBoundingRadius: function() {

			var centroidToVert = new THREE.Vector3();

			return function computeFaceBoundingRadius() {
				...
				this.radius = Math.max( centroidToVert.subVectors( centroid, va ).length(), centroidToVert.subVectors( centroid, vb ).length(), centroidToVert.subVectors( centroid, vc ).length() );
		...
		computeVertexBoundingRadius: function() {

			var sphere = new THREE.Sphere();

			return function computeVertexBoundingRadius() {

				var geometry = this.object.geometry || this.object;

				// If isPoints is true object is of type THREE.Points
				// ( previously called THREE.ParticleSystem or THREE.PointsCloud )
				if ( this.object.isPoints ) {

					this.radius = this.object.material.size;

				} else {

					sphere.setFromPoints( geometry.vertices, this.vertex );
		...
		computeBoundingRadius: function() {
		...
		distanceTo: function( point ) {
			...
			if ( this.face ) {
				...
				var face = this.face,
					faceIndex = geometry.faces.indexOf( face ),
					fvA = vertices[ face.a ],
					fvB = vertices[ face.b ],
					fvC = vertices[ face.c ];

				intersection = checkFaceIntersection( object, raycaster, ray, fvA, fvB, fvC, intersectionPoint );
		...
		// Code partly borrowed from THREE.Mesh
		raycast: function( raycaster, intersects ) {
			...
			function checkFaceIntersection( object, raycaster, ray, pA, pB, pC, point ) {
