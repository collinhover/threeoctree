...
( function () {
	...
	THREE.Ray = function () {
		...
		this.intersectOctreeObject = function ( object, recursive ) {

			var octreeObject,
				intersect,
				intersects = [];
			
			// check if object is octree search result
			...
			if ( object instanceof THREE.Particle ) {
				...
				var f, fl, faces, face, dot, scalar,
				rangeSq = this.range * this.range,
				geometry = object.geometry,
				vertices = geometry.vertices,
				objMatrix;
				
				// if octree search result has faces
				
				if ( octreeObject && octreeObject.faces ) {
					
					faces = octreeObject.faces;
					
				}
				// else default to object geometry
				else {
		...
		this.intersectOctreeObjects = function ( objects, recursive ) {

			var intersects = [];

			for ( var i = 0, l = objects.length; i < l; i ++ ) {

				Array.prototype.push.apply( intersects, this.intersectOctreeObject( objects[ i ], recursive ) );
